...
export class Webdock {
	...
	private getConfigDirPath(): string {
		...
		const isWindows = Deno.build && Deno.build.os === "windows";
		const lastSeparatorIndex = isWindows ? configPath.lastIndexOf("\\") : configPath.lastIndexOf("/");
		...
		try {
			...
			if (this.log) {
				console.error(colors.red(
					`${_getTimestamp()} - ${err.response ? "API" : "Network"} Error${err.response ? ` [${err.response.status}]` : ""}: ${err.response ? err.response.statusText : err.message}`,
				));
