import { profiles } from "../../../consts/consts.ts";
...
export async function resizeServerAction(PARENT: () => void, slug: string) {
	...
	const currentProfile = profiles.find((e) => e.slug == serverInfo.data.body.profile);
	const filteredProfiles = profiles.filter((e) => {
		return e.disk > (currentProfile?.disk ?? 0) &&
	...
	const profile = await Select.prompt({
		...
		options: filteredProfiles.map((p) => ({
			name: `${p.name.padEnd(longestName + 2)} | ${p.cpu.cores}C/${p.cpu.threads}T | ${(p.ram / 1024).toFixed(1)}GB RAM | ${(p.disk / 1024).toFixed(1)}GB Disk`,
			value: p.slug,
